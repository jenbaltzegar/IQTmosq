...
# Load data ----
post_s.1016 <- as.numeric(as.matrix(read.table("V1016I_underSelection_posterior_s.txt")))
post_h.1016 <- as.numeric(as.matrix(read.table("V1016I_underSelection_posterior_h.txt")))
post_s.1534 <- as.numeric(as.matrix(read.table("F1534C_underSelection_posterior_s.txt")))
post_h.1534 <- as.numeric(as.matrix(read.table("F1534C_underSelection_posterior_h.txt")))
...
# df
write.csv(df, "./results/byYr/unconverted/posteriorMeans_unconverted.csv")

# Plot Posteriors for Selection
png("./results/byYr/unconverted/plot_posterior_s_1016_byMo_sel.png")
plot(density(t(post_s.1016)), lwd=2, main="Posteriors for Selection \n Ile1016 under selection", xlab="s")
dev.off()

png("./results/byYr/unconverted/plot_posterior_s_1534_byMo_sel.png")
...
png("./results/byYr/unconverted/plot_posterior_h_1016_byMo_sel.png")
plot(density(t(post_h.1016)), lwd=2 ,main="Posteriors for Dominance \n Ile1016 under selection", xlab="h")
dev.off()

png("./results/byYr/unconverted/plot_posterior_h_1534_byMo_sel.png")
...
# Save plots
png("./results/byYr/unconverted/plot_2D_1016_byMo_sel.png")
image(z_1016, xlab="Selection Coefficient", ylab="Dominance", main="Ile1016 under selection")
dev.off()

png("./results/byYr/unconverted/plot_2D_1534_byMo_sel.png")
...
post_s.1016 <- as.data.frame(post_s.1016)
post_h.1016 <- as.data.frame(post_h.1016)
post_s.1534 <- as.data.frame(post_s.1534)
post_h.1534 <- as.data.frame(post_h.1534)

# rename colnames
colnames(post_s.1016) <- "V1"
colnames(post_h.1016) <- "V1"
colnames(post_s.1534) <- "V1"
colnames(post_h.1534) <- "V1"
...
boxplot_s.1534
ggsave("./results/byYr/unconverted/plot_boxplot_s.1534.png", dpi = 600)
...
boxplot_s.1016 <- ggplot(post_s.1016, aes("", V1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) 
boxplot_s.1016
ggsave("./results/byYr/unconverted/plot_boxplot_s.1016.png", dpi = 600)
...
boxplot_h.1534 <- ggplot(post_h.1534, aes("", V1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) 
boxplot_h.1534
ggsave("./results/byYr/unconverted/plot_boxplot_h.1534.png", dpi = 600)
...
boxplot_h.1016 <- ggplot(post_h.1016, aes("", V1)) +
  ...
  theme(plot.title = element_text(hjust = 0.5)) 
boxplot_h.1016
ggsave("./results/byYr/unconverted/plot_boxplot_h.1016.png", dpi = 600)

# Tiling the plots -----
boxplot_tile <- grid.arrange(boxplot_s.1016, boxplot_h.1016, boxplot_s.1534, boxplot_h.1534, nrow = 2)
ggsave(plot = boxplot_tile, "./results/byYr/unconverted/plot_boxplot_tiled.png")
