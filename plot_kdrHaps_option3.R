...
sprayColors <- c("No Pyrethroids" = "#d5dadc",
                 "Deltamethrin" = "#fcc3a7", 
                 "Cypermethrin" = "#8fd1bb",
                 "a-Cypermethrin" = "#b2d4f5",
                 "Cypermethrin & a-Cypermethrin" = "#0e8c62",
                 "Multiple*" = "#E8743B")
sprayScale <- scale_fill_manual(name = 'Pyrethroids', values = sprayColors, guide = guide_legend(override.aes = list(alpha = 1)))
...
kdrHaps <- ggplot(data = freqAll_long[!is.na(freqAll_long$Frequency),], aes(x=year, y=Frequency)) +
  ...
  # geom_rect(data = NULL, aes(xmin = 2001.5, xmax = 2014.5, ymin = -Inf, ymax = Inf, fill="Pyrethroids")) + # Pyrethroids 2002 - 2014
  geom_rect(data = NULL, aes(xmin = 1999.0, xmax = 2001.5, ymin = -Inf, ymax = Inf, fill="No Pyrethroids")) + # No pyrethroids
  geom_rect(data = NULL, aes(xmin = 2001.5, xmax = 2007.5, ymin = -Inf, ymax = Inf, fill="Deltamethrin")) +   # Delatmethrin 2002 - 2007
  geom_rect(data = NULL, aes(xmin = 2007.5, xmax = 2008.5, ymin = -Inf, ymax = Inf, fill="Cypermethrin")) +   # Cypermethrin 2008
  geom_rect(data = NULL, aes(xmin = 2008.5, xmax = 2011.5, ymin = -Inf, ymax = Inf, fill="a-Cypermethrin")) +   # a-Cypermethrin 2009 - 2011
  geom_rect(data = NULL, aes(xmin = 2011.5, xmax = 2012.5, ymin = -Inf, ymax = Inf, fill="Cypermethrin & a-Cypermethrin")) +   # Cypermethrin & a-Cypermethrin 2012
  geom_rect(data = NULL, aes(xmin = 2012.5, xmax = 2013.5, ymin = -Inf, ymax = Inf, fill="Multiple*")) +   # Lambda & Alpha & Cyper & Alpha+pyriprox in 2013 ## "Lambda, a-Cypermethrin, Cypermethrin, & a-Cypermethrin+pyriporx"
  geom_rect(data = NULL, aes(xmin = 2013.5, xmax = 2014.5, ymin = -Inf, ymax = Inf, fill="Cypermethrin")) +   # Cypermethrin 2014
  geom_rect(data = NULL, aes(xmin = 2014.5, xmax = 2018.0, ymin = -Inf, ymax = Inf, fill="No Pyrethroids")) +   # No pyrethroid
